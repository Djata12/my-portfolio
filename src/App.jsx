...
const App = () => {
  ...
  // All hooks are declared first ^^, THEN we can do conditional rendering
  ...
  const personalInfo = {
    ...
    email: "Jamindjata@gmail.com",
    github: "https://github.com/Djata12",
    linkedin: "https://www.linkedin.com/in/jamin-nii-annang-djata-4b0838283/"
  };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans ${darkMode ? 'bg-slate-950 text-slate-50' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <nav className={`fixed w-full backdrop-blur-md shadow-sm z-50 transition-all duration-300 border-b ${
        darkMode ? 'bg-slate-900/70 border-slate-800' : 'bg-white/70 border-white/20'
      }`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div key={index} className="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-xl transition duration-300">
                ...
                <div className="p-6">
                  <h3 className={`text-xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-slate-900'}`}>{project.title}</h3>
                  <p className={`mb-4 text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{project.description}</p>
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.tech.map((t, i) => (
                      <span key={i} className="px-3 py-1 bg-blue-50 text-blue-600 text-xs font-medium rounded-full">{t}</span>
                    ))}
                  </div>
